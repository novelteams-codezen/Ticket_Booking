...
{
    /// <summary>
    /// Controller responsible for managing settlement-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class SettlementController : ControllerBase
    {
        ...
        public SettlementController(TicketBookingContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new settlement to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Settlement.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Settlement>.ApplyFilter(query, filterCriteria, searchTerm);
            if (!string.IsNullOrEmpty(sortField))
            {
                var parameter = Expression.Parameter(typeof(Settlement), "b");
                var property = Expression.Property(parameter, sortField);
                var lambda = Expression.Lambda<Func<Settlement, object>>(Expression.Convert(property, typeof(object)), parameter);
        ...
        {
            var entityData = _context.Settlement.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Settlement.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Settlement updatedEntity)
        {
            ...
            this._context.Settlement.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Settlement> updatedEntity)
        {
            ...
            var existingEntity = this._context.Settlement.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Settlement.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
