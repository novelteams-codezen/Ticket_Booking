...
{
    /// <summary>
    /// Controller responsible for managing paymentaccount-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class PaymentAccountController : ControllerBase
    {
        ...
        public PaymentAccountController(TicketBookingContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new paymentaccount to the database</summary>
        ...
        {
            _context.PaymentAccount.Add(model);
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.PaymentAccount.IncludeRelated().AsQueryable();
        ...
        {
            var entityData = _context.PaymentAccount.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.PaymentAccount.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] PaymentAccount updatedEntity)
        {
            ...
            this._context.PaymentAccount.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<PaymentAccount> updatedEntity)
        {
            ...
            var existingEntity = this._context.PaymentAccount.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.PaymentAccount.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
