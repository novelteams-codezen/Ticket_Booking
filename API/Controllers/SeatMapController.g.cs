...
{
    /// <summary>
    /// Controller responsible for managing seatmap-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class SeatMapController : ControllerBase
    {
        ...
        public SeatMapController(TicketBookingContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new seatmap to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.SeatMap.IncludeRelated().AsQueryable();
        ...
        {
            var entityData = _context.SeatMap.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.SeatMap.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] SeatMap updatedEntity)
        {
            ...
            this._context.SeatMap.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<SeatMap> updatedEntity)
        {
            ...
            var existingEntity = this._context.SeatMap.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.SeatMap.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
