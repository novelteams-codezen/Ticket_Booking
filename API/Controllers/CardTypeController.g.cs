...
{
    /// <summary>
    /// Controller responsible for managing cardtype-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class CardTypeController : ControllerBase
    {
        ...
        public CardTypeController(TicketBookingContext context)
        {
            _context = context;
        }

        /// <summary>Adds a new cardtype to the database</summary>
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.CardType.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<CardType>.ApplyFilter(query, filterCriteria, searchTerm);
            if (!string.IsNullOrEmpty(sortField))
            {
                var parameter = Expression.Parameter(typeof(CardType), "b");
                var property = Expression.Property(parameter, sortField);
                var lambda = Expression.Lambda<Func<CardType, object>>(Expression.Convert(property, typeof(object)), parameter);
        ...
        {
            var entityData = _context.CardType.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.CardType.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] CardType updatedEntity)
        {
            ...
            this._context.CardType.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<CardType> updatedEntity)
        {
            ...
            var existingEntity = this._context.CardType.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.CardType.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
